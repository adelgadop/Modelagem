...
def conditions(scen, Xu, Yu, Xh, Yh, yv, lat, Nx, Ny, dx, dy, Nt, Nrx, Nry):
    """Coriolis parameter
    Args:
    ...
    for n in range(1, Nt):
        if (scen == 'scen1') | (scen == 'scen2'):
            ...
            if rad == True:
                ...
                v[n+1,:,-1] = (v[n, :, -1] - 
                                dt*f*(u[n, 1:, -1] + u[n, :-1, -1])/2 - 
                                c*dt/dy*(v[n, :, -1] - v[n, :, -2]))

                # South -> dv/dt + fu - c*dv/dy = 0
                v[n+1, :, 0] = (v[n, :, 0] - 
                                dt*f*(u[n, 1:, 0]+ u[n, :-1, 0])/2 + 
                                c*dt/dy*(v[n, :, 1] - v[n, :, 0]))
